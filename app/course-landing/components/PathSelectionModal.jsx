...
import PathSelectionContent from './PathSelectionContent';

export default function PathSelectionModal({ isOpen, onClose, onSelectBlackFriday, onSelectStateFunded }) {
  // Close on ESC key
  useEffect(() => {
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[250] flex items-center justify-center p-4 sm:p-6 bg-black/95 backdrop-blur-md"
          onClick={onClose}
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-full max-w-4xl my-4 sm:my-8 max-h-[95vh] sm:max-h-[90vh] flex flex-col"
            ...
            <div className="relative rounded-3xl border border-gold-500/30 shadow-2xl flex-1 flex flex-col min-h-0">
              ...
              >
                <PathSelectionContent
                  onSelectBlackFriday={onSelectBlackFriday}
                  onSelectStateFunded={onSelectStateFunded}
                />
              </div>
            </div>
          </motion.div>
